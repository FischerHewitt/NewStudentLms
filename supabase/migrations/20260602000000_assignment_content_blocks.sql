-- Content Blocks on assignments.
-- Adds an ordered content_blocks JSONB array to assignments and updates
-- publish_course_structure to persist it transactionally (ADR-0006 preserved).
--
-- Decision 1A: the syllabus-to-course AI is unchanged. When a preview assignment
-- carries no content_blocks, none are persisted; readers synthesize a single
-- text block from instructions at read time (see lib/content-blocks.ts).
-- Decision 2A: instructions remains the canonical text; content_blocks ride
-- alongside it.
...
begin
  ...
  loop
    ...
    loop
      ...
      v_content_blocks := coalesce(v_assignment -> 'content_blocks', '[]'::jsonb);
...
comment on function publish_course_structure(uuid, uuid, uuid, jsonb) is
  'Publishes a reviewed Course structure transactionally: Modules, Assignments (with content_blocks), Rubrics, Enrollment, then clears generation_preview.';
